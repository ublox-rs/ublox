...
use ublox_device::ublox::{
    self,
    nav_pvt::{common::NavPvtFlags2, proto31::NavPvt},
    UbxPacketRequest,
};
...
impl PkgHandler {
    ...
    // Handler method that work with NavPvtRef proto 31
    fn handle_nav_pvt_31(&mut self, pkg: &ublox::nav_pvt::proto31::NavPvtRef) {
        debug!("{pkg:?}");
        let mut pvt = to_dds_pvt_31(pkg);
        pvt.key = self.dds_key.clone();
        if let Err(e) = dds::write_sample(&self.nav_pvt_wrt, &pvt) {
            warn!("failed to write PVT message: {e} ");
        } else {
    ...
    // Handler method that work with NavPvtRef proto 27
    fn handle_nav_pvt_27(&mut self, pkg: &ublox::nav_pvt::proto27::NavPvtRef) {
        debug!("{pkg:?}");
        let mut pvt = to_dds_pvt_27(pkg);
        pvt.key = self.dds_key.clone();
        if let Err(e) = dds::write_sample(&self.nav_pvt_wrt, &pvt) {
            warn!("failed to write PVT message: {e} ");
        } else {
...
pub fn to_dds_pvt_31(pkg: &ublox::nav_pvt::proto31::NavPvtRef) -> idl::ubx::nav_pvt::NavPvt {
    let mut pvt = idl::ubx::nav_pvt::NavPvt {
        itow: pkg.itow(),
        ..Default::default()
    };
...
pub fn to_dds_pvt_27(pkg: &ublox::nav_pvt::proto27::NavPvtRef) -> idl::ubx::nav_pvt::NavPvt {
    let mut pvt = idl::ubx::nav_pvt::NavPvt {
        itow: pkg.itow(),
        ..Default::default()
    };
