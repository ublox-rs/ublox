...
lint *ARGS:
    cargo fmt --all
    cargo clippy -p ublox -p ublox_derive --all-features --all-targets -- -D warnings

# Run clippy lints with version 1.88.0
[group("misc")]
lint-msrv *ARGS:
    cargo fmt --all
    cargo +1.88 clippy -p ublox -p ublox_derive --all-features --all-targets -- -D warnings
