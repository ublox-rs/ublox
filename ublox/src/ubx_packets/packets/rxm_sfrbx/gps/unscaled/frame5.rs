...
#[derive(Debug, Default, Clone)]
pub struct GpsUnscaled5Word5 {
    pub omega_dot: u16,
    pub sv_health: u8,
}

impl GpsUnscaled5Word5 {
    pub(crate) fn decode(dword: u32) -> Self {
        let dword = dword >> GPS_PARITY_SIZE;
...
#[derive(Debug, Default, Clone)]
pub struct GpsUnscaled5Word6 {
    /// 24-bit SQRT(a)
    pub sqrt_a: u32,
}

impl GpsUnscaled5Word6 {
    pub(crate) fn decode(dword: u32) -> Self {
        let dword = dword >> GPS_PARITY_SIZE;
...
#[derive(Debug, Default, Clone)]
pub struct GpsUnscaled5Word7 {
    /// 24-bit omega0
    pub omega0: u32,
}

impl GpsUnscaled5Word7 {
    pub(crate) fn decode(dword: u32) -> Self {
        let dword = dword >> GPS_PARITY_SIZE;
...
#[derive(Debug, Default, Clone)]
pub struct GpsUnscaled5Word8 {
    /// 24-bit omega
    pub omega: u32,
}

impl GpsUnscaled5Word8 {
    pub(crate) fn decode(dword: u32) -> Self {
        let dword = dword >> GPS_PARITY_SIZE;
...
#[derive(Debug, Default, Clone)]
pub struct GpsUnscaled5Word9 {
    /// 24-bit m0
    pub m0: u32,
}

impl GpsUnscaled5Word9 {
    pub(crate) fn decode(dword: u32) -> Self {
        let dword = dword >> GPS_PARITY_SIZE;
...
#[derive(Debug, Default, Clone)]
pub struct GpsUnscaled5Word10 {
    pub af0: u16,
    pub af1: u16,
}

impl GpsUnscaled5Word10 {
    pub(crate) fn decode(dword: u32) -> Self {
        let dword = dword >> (GPS_PARITY_SIZE + 2);

        let mut af0 = ((dword & WORD10_AF0LSB_MASK) >> WORD10_AF0MSB_SHIFT) as u16;
        let af1 = ((dword & WORD10_AF1_MASK) >> WORD10_AF1_SHIFT) as u16;
        af0 |= (dword & WORD10_AF0LSB_MASK) as u16;
        Self { af0, af1 }
    }
}
